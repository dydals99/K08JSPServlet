...
public class MVCBoardDAO extends DBConnPool {
	...
	/*
	 * 모델1 방식에서는 board테이블 및 BoardDTO클래스를 사용했지만 모델2 방식에서는 mvcboard테이블 및 MVCBoardDTO를
	...
	public boolean confirmPassword(String pass, String idx) {
		boolean isCorr = true;
		try {
			// 일변번호와 패스워드가 일치하는 게시물이 있는지 확인한다.
	...
	public int updatePost(MVCBoardDTO dto) {
		...
		try {
			// 쿼리문 템플릿 준비
			// 일련번호와 패스워드까지 where절에 추가형 둘다 일치할때만
			// 수정처리된다.
