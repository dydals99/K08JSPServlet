...
속성정리 
 ...
 -->
	...
	eq를 통해 값에대한 비교와 문자열에 대한 비교 모두를 할수있다.
	여기서 사용한 eq는 Java의 compareTo와 같이 사전순으로 문자열을
	비교한다. -->
	...
	받은 후 tjoeun/1234인 경우 'tjoeun님, 하이룽~' 이라고 출력한다.
	만약 틀렸다면 "아이디/비번을 확인하세요"라고 출력한다.
	EL과 JSTL의 if 탵그만을 이용해서 구현하시오.
	 -->
...
<%-- 	 내가 한거
	...
	<c:if test="${ not empty param.user }">
		<c:if test='${ id eq "tjoeun"and pass eq "1234"}' var="login">
			tjoeun님, 하이룽~ <br>
		</c:if>
		<c:if test="${ not login }">
			아이디/비번을 확인하세요 <br /> 
